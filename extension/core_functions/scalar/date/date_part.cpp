...
ScalarFunctionSet YearFun::GetFunctions() {
	...
	set.SetUnaryArgProperties(ArgProperties().Increasing(false).RequiresFinite());
	return set;
}
...
ScalarFunctionSet EpochNsFun::GetFunctions() {
	...
	operator_set.SetUnaryArgProperties(ArgProperties().Increasing(false).RequiresFinite());
	return operator_set;
}
...
ScalarFunctionSet MakeTimestampMsFun::GetFunctions() {
	...
	operator_set.AddFunction(
	    ScalarFunction({LogicalType::BIGINT}, LogicalType::TIMESTAMP, DatePart::EpochMillisOperator::Inverse));
	operator_set.SetUnaryArgProperties(ArgProperties().Increasing(false));
	return operator_set;
}
