...
void ParquetMetaDataOperator::BindSchema<ParquetMetadataOperatorType::SCHEMA>(vector<LogicalType> &return_types,
	...
	names.emplace_back("flattened_column_id");
	return_types.emplace_back(LogicalType::BIGINT);
}
...
void ParquetSchemaProcessor::InitializeInternal(ClientContext &context) {
	column_id = 0;
}
...
void ParquetSchemaProcessor::ReadRow(DataChunk &output, idx_t output_idx, idx_t row_idx) {
	...
	// column_id
	auto is_leaf = !column.__isset.num_children || column.num_children == 0;
	output.SetValue(12, output_idx, ParquetElementBigint(column_id, is_leaf));
	if (is_leaf) {
		column_id++;
	}
	// flattened_column_id
	output.SetValue(13, output_idx, Value::BIGINT(output_idx));
}
