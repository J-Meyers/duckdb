...
BoundCastInfo DefaultCasts::DecimalCastSwitch(BindCastInput &input, const LogicalType &source,
	...
	const auto decimal_to_int = ArgProperties().Increasing(/*strict=*/false);
	switch (target.id()) {
	...
	case LogicalTypeId::DECIMAL: {
		...
		const bool strict = DecimalType::GetScale(target) >= DecimalType::GetScale(source);
		return BoundCastInfo(fn).SetArgProperties(ArgProperties().Increasing(strict));
	}
	case LogicalTypeId::FLOAT:
		...
		return BoundCastInfo(FromDecimalCast<float>).SetArgProperties(ArgProperties().Increasing(/*strict=*/false));
	case LogicalTypeId::DOUBLE:
		return BoundCastInfo(FromDecimalCast<double>).SetArgProperties(ArgProperties().Increasing(/*strict=*/false));
