...
BoundCastInfo MakeNumericToNumericCast() {
	...
	if (IsOrderPreservingNumericCast<SRC, DST>()) {
		info.SetArgProperties(ArgProperties().Increasing(/*strict=*/!HasFloatPrecisionLoss<SRC, DST>()));
	}
	return info;
}
...
BoundCastInfo MakeNumericToDecimalCast() {
	...
	const bool strict = !std::is_floating_point<SRC>::value;
	info.SetArgProperties(ArgProperties().Increasing(/*strict=*/strict));
	return info;
}
} // namespace
